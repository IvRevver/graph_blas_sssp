 ...
 * @return GrB_SUCCESS если успешно
 */
static GrB_Info parse_dimensions(const char *line, GrB_Index *nrows, GrB_Index *ncols,
                                 GrB_Index *nentries) {
 ...
 * @return GrB_SUCCESS если успешно
 */
static GrB_Info build_adjacency_matrix(GrB_Matrix A, GrB_Index *rows, GrB_Index *cols, double *vals,
                                       GrB_Index nentries) {
    if (!A || !rows || !cols || !vals) {
        return GrB_NULL_POINTER;
    }
...
GrB_Info graph_load(LAGraph_Graph *graph, const char *filename, GraphInfo *info) {
    ...
    LAGraph_Kind kind =
        info->is_symmetric ? LAGraph_ADJACENCY_UNDIRECTED : LAGraph_ADJACENCY_DIRECTED;
