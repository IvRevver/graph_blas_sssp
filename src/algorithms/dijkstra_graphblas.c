    ...
    *priority = pq->priorities[0];
    ...
    if (pq->size > 0) {
        ...
        while (2 * i + 1 < pq->size) {
            ...
            if (pq->priorities[i] <= pq->priorities[smallest]) break;
...
GrB_Info dijkstra_graphblas(SSSP_Result *result, LAGraph_Graph graph, 
    ...
    while (1) {
        ...
        info = GrB_vxm(d_neighbors, NULL, NULL, minplus_semiring, 
                       u_mask, graph->A, NULL);
